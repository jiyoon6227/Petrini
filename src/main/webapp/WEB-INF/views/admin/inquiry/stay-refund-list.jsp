...
<%-- 2026/07/31 장우철 — 관리자 숙소 환불신청 목록 (숙소 예약 관리 UI 통일) --%>
...
<main class="adm-main">
  <div class="adm-page-head">
    <div class="adm-page-head-left">
      ...
      <p class="adm-page-desc">체크인 이후 환불 신청을 조회하고 승인·거절을 처리하세요.</p>
    </div>
  </div>
  ...
  <div style="display:flex;gap:0;border-bottom:2px solid #E4E6ED;margin-bottom:16px;flex-wrap:wrap">
    <a href="${contextPath}/admin/inquiry/stay-refund?status=ALL&amp;keyword=${keyword}"
    ...
    <a href="${contextPath}/admin/inquiry/stay-refund?status=WAIT&amp;keyword=${keyword}"
      ...
      대기 <span style="background:#F3F4F6;font-size:11px;padding:1px 7px;border-radius:20px;margin-left:4px">${statusCounts.WAIT}</span>
    </a>
    <a href="${contextPath}/admin/inquiry/stay-refund?status=DONE&amp;keyword=${keyword}"
      ...
      처리완료 <span style="background:#F3F4F6;font-size:11px;padding:1px 7px;border-radius:20px;margin-left:4px">${statusCounts.DONE}</span>
    </a>
  </div>

  <form method="get" action="${contextPath}/admin/inquiry/stay-refund" style="margin-bottom:16px;display:flex;gap:8px">
    <input type="hidden" name="status" value="${status}">
    <input type="text" name="keyword" value="${keyword}" placeholder="신청번호·예약번호·회원명·숙소명"
    ...
    <table class="adm-table" style="width:100%;border-collapse:collapse;font-size:13px">
      <thead>
        <tr style="background:#FAFBFC;text-align:left">
          ...
          <th style="padding:12px">처리상태</th>
          <th style="padding:12px"></th>
        </tr>
      </thead>
      <tbody>
        <c:if test="${empty list}">
          <tr><td colspan="7" style="padding:32px;text-align:center;color:#999">신청이 없습니다.</td></tr>
        </c:if>
