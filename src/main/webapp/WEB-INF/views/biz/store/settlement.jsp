...
<%--
  2026/08/05 장우철 — 쇼핑 정산 내역 (S9 + S10)
  - 2-1 상단 요약 / 2-2 목록 / 2-3·2-4 상세
  - S10 중간정산 요청 버튼·폼
--%>
<style>
...
<main class="biz-main">
  <div class="biz-page-head settle-page-head">
    <div>
      ...
      <p class="biz-page-desc">월정산·중간정산 매출 및 지급 내역을 확인하세요. 행의 ▶ 로 주문상품 상세를 볼 수 있습니다.</p>
    </div>
  ...
  <div class="biz-card" style="margin-bottom:16px">
    <form class="settle-filter" method="get" action="${contextPath}/biz/store/settlement">
      ...
      <%-- 2026/08/06 장우철 — 필터 줄 오른쪽 정산계좌 버튼 --%>
...
<div class="adhoc-modal-bg" id="adhocModalBg" onclick="if(event.target===this) closeAdhocModal()">
  <div class="adhoc-modal" role="dialog" aria-labelledby="adhocModalTitle">
    ...
    <div class="adhoc-modal-body">
      <div class="adhoc-note">
        시작일은 <b>컷오프가 속한 달의 1일</b>로 고정됩니다. 관리자 승인 후 정산 마스터가 생성됩니다.<br>
        요청대기(REQUESTED)는 사업자당 <b>1건만</b> 허용됩니다.
      </div>

      <div class="adhoc-row">
        <label>정산 범위<span class="req">*</span></label>
        <div class="adhoc-scope">
          <label><input type="radio" name="adhocScope" value="ALL" checked onchange="toggleAdhocProduct()"> 상품 전체 (ALL)</label>
          <label><input type="radio" name="adhocScope" value="PRODUCT" onchange="toggleAdhocProduct()"> 특정 상품 (PRODUCT)</label>
        </div>
      </div>

      <div class="adhoc-row" id="adhocProductRow" style="display:none">
        <label>상품 선택<span class="req">*</span></label>
        <select id="adhocProductId">
          <option value="">상품을 선택하세요</option>
          <c:choose>
            <c:when test="${empty productList}">
              <option value="" disabled>등록된 상품이 없습니다</option>
            </c:when>
            <c:otherwise>
              <c:forEach var="p" items="${productList}">
                <option value="${p.productId}"><c:out value="${p.productName}"/> (#${p.productId})</option>
              </c:forEach>
            </c:otherwise>
          </c:choose>
        </select>
      ...
      <div class="adhoc-note">
        · 집계: 기간 내 <b>구매확정 + 미정산</b> 주문상품만 · 환불중/완료 제외<br>
        · 이미 정산된 ORDER_ITEM 은 자동 제외
      </div>
    </div>
    <div class="adhoc-modal-foot">
...
<script>
  ...
  function verifySettleAccount() {
    var $opt = document.getElementById('saBankName');
  ...
  function saveSettleAccount() {
    ...
    btn.disabled = true;
    csrfFetch(CTX + '/biz/store/settlement/account', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json' },
      body: fd.toString()
    })
  ...
  function toggleAdhocProduct() {
    var scope = document.querySelector('input[name="adhocScope"]:checked');
    var row = document.getElementById('adhocProductRow');
    row.style.display = (scope && scope.value === 'PRODUCT') ? '' : 'none';
  }
  function submitAdhocRequest() {
    ...
    var productId = document.getElementById('adhocProductId').value;
    var end = document.getElementById('adhocEnd').value;
    ...
    btn.disabled = true;
    csrfFetch(CTX + '/biz/store/settlement/request', {
      method: 'POST',
  ...
  function toggleSettleDetail(btn) {
    ...
    fetch(CTX + '/biz/store/settlement/items?settleId=' + encodeURIComponent(settleId), {
      ...
      .then(function (data) {
        ...
        if (items.length === 0) {
          wrap.innerHTML = '<div class="settle-detail-empty">포함된 주문상품 상세가 없습니다.</div>';
          wrap.setAttribute('data-loaded', 'Y');
          return;
        }
        ...
        var html = '<table class="settle-detail-table"><thead><tr>'
          ...
          + '<th>상품매출</th><th>택배비</th><th>수수료</th><th>정산금</th><th>상태</th>'
          + '</tr></thead><tbody>';
        ...
        items.forEach(function (it) {
          html += '<tr>'
            ...
            + '<td>' + fmtWon(it.itemSalesAmount) + '</td>'
            + '<td>' + fmtWon(it.deliveryFeeAmount) + '</td>'
